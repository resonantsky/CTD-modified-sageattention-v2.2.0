...
_SAGE_LOGGED_SHAPES: set = set()  # shapes written to sage.log this session
_SAGE_STEP_CALLS = 0
_SAGE_SEEN_PATHS: set = set()
_SAGE_LAST_TIME = 0.0
...
def _sage_log(func_name, q, k, v, tensor_layout, is_causal, dtype, path_label):
    ...
    if now - _SAGE_LAST_SPIN > 0.06:
        ...
        if tensor_layout == "varlen":
            ...
            shape_str = f"{q.size(0)}×{q.size(1)}×{q.size(2)}×{q.size(3)}"
        # Active config from sageattention.triton.config
        try:
            from sageattention.triton.config import BLOCK_M, BLOCK_N, get_launch_params
            _lp = get_launch_params(q.size(-1), is_causal)
            cfg_str = (f"BM{BLOCK_M} BN{BLOCK_N}"
                       f" nw{_lp['num_warps']} wpe{_lp.get('waves_per_eu', 0)} ns{_lp['num_stages']}")
        except Exception:
            cfg_str = "?"
...
@functools.cache
def _is_rocm() -> bool:
    """True when running under ROCm/HIP (AMD GPU)."""
    return getattr(torch.version, "hip", None) is not None

@functools.cache
def _get_rdna_block_sizes():
    """Return (BLKQ, BLKK) tuned for RDNA warp size 32.  Falls back to
    NVIDIA defaults (128/64) on non-RDNA or CUDA hardware."""
    if not _is_rocm():
        return 128, 64
    try:
        gcn = torch.cuda.get_device_properties(0).gcnArchName
        if gcn.startswith("gfx1"):   # RDNA 1/2/3/4 — warp_size=32, best: BM=32 BN=16
            return 32, 16
    except Exception:
        pass
    return 32, 16   # default for any ROCm device
...
def sageattn(
    ...
    **kwargs: Any,
    ...
    # ROCm/AMD: all CUDA C extensions are NVIDIA-only; route to pure-Triton path
    smooth_k = kwargs.pop("smooth_k", True)
    if _is_rocm():
        # logging is emitted inside sageattn_qk_int8_pv_fp16_triton
        return sageattn_qk_int8_pv_fp16_triton(q, k, v, tensor_layout=tensor_layout, is_causal=is_causal, sm_scale=sm_scale, smooth_k=smooth_k, return_lse=return_lse)

    arch = get_cuda_arch_versions()[q.device.index]
    if arch == "sm80":
        _sage_log("sageattn", q, k, v, tensor_layout, is_causal, q.dtype, f"CUDA-{arch}")
        ...
        _sage_log("sageattn", q, k, v, tensor_layout, is_causal, q.dtype, f"CUDA-{arch}-fp8")
...
def sageattn_qk_int8_pv_fp16_triton(
    ...
    **kwargs: Any,
    ...
    if quantization_backend == "triton":
        _blkq, _blkk = _get_rdna_block_sizes()  # RDNA: 32/16, NVIDIA: 128/64
        q_int8, q_scale, k_int8, k_scale = per_block_int8_triton(q, k, km=km, sm_scale=sm_scale, tensor_layout=tensor_layout, BLKQ=_blkq, BLKK=_blkk)
...
def sageattn_varlen(
    ...
    **kwargs: Any,
    ...
    if is_causal:
        _sage_log("sageattn_varlen", q, k, v, "varlen", is_causal, dtype, f"INT8 varlen-causal (max_q={max_seqlen_q})")
        ...
        _sage_log("sageattn_varlen", q, k, v, "varlen", is_causal, dtype, f"INT8 varlen-noncausal (max_q={max_seqlen_q})")
...
def sageattn_qk_int8_pv_fp16_cuda(
    ...
    **kwargs: Any,
    ...
    dtype = q.dtype
    # ROCm: fall back to Triton path — CUDA C extensions are NVIDIA-only
    if _is_rocm():
        return sageattn_qk_int8_pv_fp16_triton(q, k, v, tensor_layout=tensor_layout, is_causal=is_causal, sm_scale=sm_scale, smooth_k=smooth_k, return_lse=return_lse)
