...
from .config import BLOCK_M, BLOCK_N, get_launch_params
...
def forward(q, k, v, q_scale, k_scale, tensor_layout="HND", output_dtype=torch.float16, return_lse=False):
    # STAGE=3 is always required: causal kernel calls _attn_fwd_inner with
    # (4 - STAGE = 1) for pre-diagonal blocks, then STAGE=2 for the diagonal.
    stage = 3
    ...
    _attn_fwd[grid](
        ...
        **get_launch_params(head_dim, causal=True))
